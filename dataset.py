...
def find_min_max_zero_indices(tensor):
...
MAX_LENGTH = cal_max_frame_len(config.sampling_rate, config.train_seq_len)
...
class Stronglabeled_Dataset(torch.utils.data.Dataset):                           
    def __init__(self, data_folder, recording_paths, timings, murmur_labels, outcome_labels, 
    ...
    def __getitem__(self, idx):
        ...
        if self.clean_noise:
            ...
            min_idx, max_idx = find_min_max_zero_indices(columns)
...
def get_dataframe_dict(config):
    ...
    # external_data_folder = config.external_datapath
    ...
    patient_df = load_patient_files(config.train_datapath, stop_frac= split_ratio)
    ...
    # External_df
    weaklabeled_df, unlabeled_df = get_external_df(config.external_datapath, config.external_data_subpath)
    concat_unlabeld_df = pd.concat([weaklabeled_df, unlabeled_df], axis= 0).reset_index(drop=True)    

    # Test df
    test_patient_df = load_patient_files(config.test_datapath, stop_frac= split_ratio)
